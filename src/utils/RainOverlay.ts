...
/* ── Config (tuned slightly for Star-Riders globe scale) ──────────────── */

const STREAK_COUNT = 180;
const WIND_ANGLE = 0.32;
const ANGLE_JITTER = 0.1;
...
void main() {
  ...
  gl_FragColor = vec4(0.78, 0.82, 0.90, shape * opacity);
}
`;

/* ── Glass droplets shader (adapted from Shadertoy, for wet lens effect) ─ */
...
/* ── RainOverlay class (globe-mode only visuals) ─────────────────────── */
...
export class RainOverlay {
  ...
  constructor() {
    ...
    this.lightningMat = new ShaderMaterial({
      ...
      fragmentShader: `uniform float alpha; void main() { gl_FragColor = vec4(0.88, 0.90, 0.95, alpha); }`,
  ...
  /** Call when switching between surface chase and space overview for visual tuning */
  setSpaceViewMode(isSpace: boolean) {
    this.isSpaceView = isSpace;
  }
  ...
  private spawnStreak(idx: number, heavy = false) {
    ...
    s.length = heavy ? 0.11 + Math.random() * 0.18 : 0.07 + Math.random() * 0.13;
    s.width = heavy ? 0.0018 + Math.random() * 0.0018 : 0.0014 + Math.random() * 0.0014;
    s.speed = heavy ? 2.2 + Math.random() * 1.6 : 1.7 + Math.random() * 1.3;
    s.x = (Math.random() - 0.5) * 2.7;
    s.y = 1.12 + Math.random() * 0.28;
    s.active = true;
  }
  ...
  private updateLightning(dt: number, rainWeight: number) {
    ...
    if (this.lightningCooldown <= 0) {
      ...
      this.lightningAlpha = 0.48 + Math.random() * 0.32;
      this.lightningMat.uniforms.alpha.value = this.lightningAlpha;
      this.lightningMesh.visible = true;
      const minInterval = 2.0 - urgency * 1.1;
      const maxInterval = 5.8 - urgency * 2.8;
      this.lightningCooldown = minInterval + Math.random() * (maxInterval - minInterval);
      this.onLightningFlash?.();

      if (Math.random() < 0.38) {
        setTimeout(() => {
          this.lightningAlpha = 0.28 + Math.random() * 0.18;
        }, 75 + Math.random() * 110);
      }
    }
  }

  update(dt: number, rainWeight: number, moonProgress = 0, isSpaceView = false) {
    this.moonProgress = moonProgress;
    this.currentWeight = rainWeight;
    this.isSpaceView = isSpaceView;
    ...
    const spawnChance = apocalypse ? 1.0 : intensity * 0.82;
    const spawnRate = apocalypse ? 58 : 28;
    const activeLimit = apocalypse ? STREAK_COUNT : Math.floor(STREAK_COUNT * 0.28);
    const opacityMul = apocalypse ? 0.52 : (isSpaceView ? 0.22 : 0.32);
    ...
    const glassOpacity = isSpaceView ? intensity * 0.18 : intensity * 0.9;
    this.glassMat.uniforms.time.value = this.time;
    this.glassMat.uniforms.opacity.value = glassOpacity;
