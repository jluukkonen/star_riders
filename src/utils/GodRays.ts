...
void main() {
  ...
  p.xz = vec2(p.x * c - p.z * s, p.x * s + p.z * s);
...
export class GodRays {
  ...
  constructor() {
    // Open-ended cone: radiusTop=0.5 (near sun), radiusBottom=80.0 (near globe), height=180.0 (scaled for Star-Riders)
    const geo = new CylinderGeometry(5, 80, 180, 32, 1, true);
    // Offset geometry so origin is at the top tip (near the sun)
    geo.translate(0, -90.0, 0);
