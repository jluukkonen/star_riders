...
export function terrainNoise(
  ...
  // A complete procedural Fractional Brownian Motion (FBM) with coordinate-level domain warping
  const fbm = (px: number, py: number, pz: number, octs: number) => {
    let value = 0;
    let amplitude = 1.0;
    let freq = scale;
    let maxAmp = 0;
    for (let o = 0; o < octs; o++) {
      value += noise(px * freq, py * freq, pz * freq) * amplitude;
      maxAmp += amplitude;
  ...
  // Domain warping offsets create organic tectonic lines, ridges and fault clefts
  const warpStrength = 0.38;
  
  // Layer 1 warp offsets
  const qx = fbm(x + 1.25, y + 3.42, z + 5.67, Math.max(1, octaves - 2));
  const qy = fbm(x + 4.13, y + 1.89, z + 6.31, Math.max(1, octaves - 2));
  const qz = fbm(x + 5.51, y + 2.22, z + 1.11, Math.max(1, octaves - 2));

  // Layer 2 deeper warp details
  const rx = fbm(x + qx * warpStrength + 0.73, y + qy * warpStrength + 2.91, z + qz * warpStrength + 1.45, Math.max(1, octaves - 2));
  const ry = fbm(x + qx * warpStrength + 1.34, y + qy * warpStrength + 3.82, z + qz * warpStrength + 0.56, Math.max(1, octaves - 2));
  const rz = fbm(x + qx * warpStrength + 2.51, y + qy * warpStrength + 1.49, z + qz * warpStrength + 3.88, Math.max(1, octaves - 2));

  // Final warped lookup position using the primary coordinate offsets
  const wx = x + rx * warpStrength * 1.6;
  const wy = y + ry * warpStrength * 1.6;
  const wz = z + rz * warpStrength * 1.6;
